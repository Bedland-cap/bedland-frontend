...
describe('SidebarItem', () => {
  const item = ({ iconName, label, linkTo }: SidebarItemProps) =>
    render(<SidebarItem label={label} iconName={iconName} linkTo={linkTo} />, {});

  it('should renders correctly sidebarItem for Buildings', () => {
    expect(item({ iconName: 'building', label: 'buildings', linkTo: '#' })).toMatchSnapshot();
  });

  it('should renders correctly sidebarItem for Residents', () => {
    expect(item({ iconName: 'residents', label: 'residents', linkTo: '#' })).toMatchSnapshot();
  });

  it('should renders correctly sidebarItem for Dashboard', () => {
    expect(item({ iconName: 'dashboard', label: 'dashboard', linkTo: '#' })).toMatchSnapshot();
  });

  it('should renders correctly sidebarItem for Wall', () => {
    expect(item({ iconName: 'wall', label: 'wall', linkTo: '#' })).toMatchSnapshot();
  });

  it('should renders correctly sidebarItem for Reports', () => {
    expect(item({ iconName: 'reports', label: 'reports', linkTo: '#' })).toMatchSnapshot();
  });

  it('should renders correctly sidebarItem for Messages', () => {
    expect(item({ iconName: 'messages', label: 'messages', linkTo: '#' })).toMatchSnapshot();
  });

  it('should renders correctly sidebarItem for Payments', () => {
  ...
  it('should renders correctly sidebarItem for Flats', () => {
